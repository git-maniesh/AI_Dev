...
function TypingIndicator() {
  return (
    <div className="flex items-center gap-1 text-sm text-gray-600 p-2">
      ...
      <p className="ml-2 text-xs opacity-70">AI is typing...</p>
    </div>
  )
}
      ...
      if (data.sender._id === 'ai') {
        ...
        try {
          ...
          setMessages(prev => {
            ...
            if (last && last.sender._id === 'ai') {
              // Append to ongoing AI message
              const updated = [...prev]
              const merged = {
                ...last,
                message: JSON.stringify({
                  text: (JSON.parse(last.message).text || '') + (messageObj.text || ''),
                  fileTree: messageObj.fileTree || JSON.parse(last.message).fileTree
                })
              }
          ...
          // Simulate typing end delay
          clearTimeout(window.__aiTypingTimeout)
          window.__aiTypingTimeout = setTimeout(() => setIsAITyping(false), 1200)
        } catch (err) {
          console.error("Error parsing AI message:", err, data.message)
        }
      } else {
      ...
      <section className="left relative flex flex-col h-screen min-w-96 bg-slate-300">
        ...
        <div className="conversation-area pt-14 pb-10 flex-grow flex flex-col h-full relative">
          <div
            ...
            {messages.map((msg, i) => (
              <div key={i} className={`${msg.sender._id === 'ai' ? 'max-w-80' : 'max-w-52'} ${msg.sender._id == user._id.toString() && 'ml-auto'} message flex flex-col p-2 bg-slate-50 w-fit rounded-md`}>
